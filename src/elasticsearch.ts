...
const log: Logger = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'Chat ElasticSearch Server', 'debug');
...
export async function checkConnection(): Promise<void> {
    ...
    while (!isConnected) {
        try {
            ...
            log.info(`Chat ElasticSearch Server ElasticSearch health status: ${response.status}`);
            isConnected = true;
        } catch (error) {
            log.error("Failed to connect to ElasticSearch", error);
            log.log('error', 'Chat ElasticSearch Server checkConnection() Method', error);
        }
    }
}

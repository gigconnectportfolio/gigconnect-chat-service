...
const log: Logger = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'Chat Service RabbitMQ Connection', 'debug');
...
export async function createConnection(): Promise<Channel | undefined> {
    try {
        ...
        log.log('error', `❌ Chat Service createConnection() Method`, error);
        return undefined;
    }
}

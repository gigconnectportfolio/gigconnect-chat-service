...
import * as chatService from '../../services/message.service'
...
describe('Chat Controller', () => {
    ...
    describe('message method', () => {
        it('should throw an error for invalid schema data', async () => {
            ...
            jest.spyOn(messageSchema, 'validate').mockImplementation((): any => Promise.resolve({
                error: {
                    name: 'ValidationError', isJoi: true, details: [{message: 'This is an error message'}]
                }
            }));

            message(req, res, next).catch(() => {
        ...
        it('should return correct json response', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith({
                message: 'Message sent successfully', conversationId: messageDocument.conversationId, messageData: messageDocument
            });
        });
    });
});
